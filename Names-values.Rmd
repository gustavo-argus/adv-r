...
* A seção \@ref(o-basico-sobre-vinculacao) apresenta a diferença entre
...
## O básico sobre Vinculação {#o-basico-sobre-vinculacao}
...
Você aprenderá mais sobre as convenções usadas neste diagrama na Seção
...
### Listas {#list-references}
\indexc{ref()}
\index{lists}

Não apenas nomes (i.e. variáveis) são usados para vincular valores; elementos de listas o fazem também. Considere a lista abaixo que, na superfície, é bem similiar ao vetor numérico acima:

```{r list1}
l1 <- list(1, 2, 3)
```

Essa lista é mais complexa, pois no lugar de armazenar os próprios valores ela
armazena referências à eles:

```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("diagrams/name-value/list.png")
```

Isso é particularmente importante quando modificamos uma lista:
...
Assim como vetores, listas usam o comportamento copiar-ao-modificar. 
A lista original permanece a mesma e R cria uma cópia modificada.
Entretando, essa é uma cópia __superficial__: o objeto dado pela lista e suas vinculações são copiadas, mas os valores referenciados por essas vinculações não são copiados. O oposto de uma cópia superficial é uma cópia profunda, onde os 
conteúdos de cada referência são copiados. Antes da versão 3.1.0 do R, cópias eram
sempre consideradas cópias profundas.

Para ver os valores que são compartilhados entre as listas, use `lobstr::ref()`. `ref()` retorna o endereço na memória para cada objeto juntamente com um identificador local para que você possa facilmente cruzar as referências dos componentes compartilhados.

```{r list4, dependson = "list3"}
ref(l1, l2)
```

### Estuturas de dados (*Data frames*) {#df-modify}

Estuturas de dados (ou *data frames*) são listas de vetores, então o comportamento copiar-ao-modificar tem consequências importantes quando você modifica um data frame. 
Veja o exemplo deste data frame:
...
Se você modificar uma coluna, apenas __essa__ coluna precisará ser modificada. As demais colunas continuarão apontando para suas referências originais:
...
Por outro lado, se você modificar uma linha, todas as colunas serão modificadas. Isso significa que cada coluna será copiada:
...
O último lugar em que R usa referências é com vetores de caracteres[^character-vector]. Geralmente, eu desenho vetores de caracteres assim:

[^character-vector]: De forma confusa, um vetor de caracteres é um vetor de *strings*, e não de caracteres individuais.
