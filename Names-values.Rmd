...
* Entenda melhor as ferramentas de programação funcional do R.
...
capítulo. Você pode encontrar as respostas no final do capítulo na Seção \@ref(names-values-answers).
...
O que faz com que `1`, `2` e `3` sejam desafiadores como nomes de variáveis?
...
2.  No código a seguir, `y` ocupa quanta memória?
...
3.  No exemplo a seguir, em qual linha `a` é copiada?
...
* A seção \@ref(binding-basics) apresenta a diferença entre
  nomes e valores e discute como `<-` cria uma ligação, ou referência,
  entre um nome e um valor.

* A seção \@ref(copy-on-modify) descreve quando o R faz uma cópia: sempre que 
  você modificar um vetor, você - quase sempre - estará criando um novo vetor 
...
`?Memory` and `?gc`), da seção [memory profiling](http://cran.r-project.org/doc/manuals/R-exts.html#Profiling-R-code-for-memory-use) do documento _Writing R extensions_ [@r-exts], e da seção [SEXPs](http://cran.r-project.org/doc/manuals/R-ints.html#SEXPs) do documento
